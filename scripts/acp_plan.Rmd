...
# America's College Promise Free Community College Plan
...
Which institutions in each state will be eligible?
How much will the plan provide? 
How much will states have to pay?
What will be the effective subsidy among all college students by state?
Which states will see increases or decreases in per-student funding levels under the provision of the plan?
Which states will gain or lose revenues based on participating in the plan. 

Below are my current  provisional answers to those questions.

The basic concept of the plan is this. The federal government will provide a subsidy to each state equal to eight percent of  
...
Given that this says all states I'm going to assume that they mean the median of all *average* state tuitions. That is, I take the average unweighted tuition in each state, then take the median of that measure for all states.  It looks like that's Connecticut, with an average in state CC tuition of \$3,984. Part of this depends on how Alaska is handled, but it's either Connecticut at \$3,984 or West Virginia at \$4,000, so \$4,000 seems most likely. 
```{r}
...
cc_inst<-cc_inst%>%
  group_by(name)%>%
  mutate(median_tuition=3984)
  
```

## How much will states get? 

When fully phased in, states will receive \$3,200 per full time equivalent student in community colleges enrolled in their first sixty credit hours. 
...
NEW total revenues will consist of state appropriations plus local appropriations plus federal funding (~3200 per student) plus state match (~800 per student once fully funded). There's a maintenance of effort provision so states are mandated not to decrease funding. 
```{r}
cc_totals<-cc_inst%>%
...
spend_sum<-cc_inst%>%
  ...
  summarize(across(.cols=c("fteug","tuition_revs","state_approps","local_approps","fed_spend","state_match"),.fns=sum,na.rm=TRUE ))%>%
  mutate(across(c("tuition_revs","state_approps","local_approps","fed_spend","state_match"), .fns= ~.x/fteug  ))%>%
  mutate(state_match_plus_tuition_revs=tuition_revs+state_match)%>%
  mutate(diff=fed_spend-state_match_plus_tuition_revs)%>%
  arrange(-diff)%>%
  mutate(name=fct_reorder(name,diff))%>%
  ungroup()%>%
  select(name,fed_spend,state_match_plus_tuition_revs,diff)
  knitr::kable(spend_sum)
```

From this perspective, it looks like there are about 35 states that would have to increase funding for community college students in order to participate. Of these, 9 would have to increase funding by $500 or so, 5 would have to increase funding by 500-1000, 15 would have to increase funding by ~1000-2000 and 6 would have to increase funding by more than 2000 if they wanted per-student subsidy levels at community colleges to remain the same.
